...
| **D33** | **La galería es contenido, no catálogo** | Lo natural sería colgar cada pieza de un `product`, y dejaría fuera justo lo que trae encargos: en el archivo de Felicitas hay una carpeta **«Papelería»** —invitaciones, *seating plans*— que no está en el catálogo. La categoría es texto acotado en el Form Request, no una clave foránea. La otra mitad de la decisión es **quién la llena**: la gestiona ella desde el backoffice, como D5 hizo con el catálogo, y aquí pesa más porque su archivo son **207 MB de fotos sin curar** —de móvil, a 3024×4032, alguna de 65 MB, con nombres de exportación de WhatsApp— y elegir cuáles son «la galería» es una decisión artística que cambia con el tiempo, no un dato que pueda fijar el código. `GET /api/gallery` es **público y sin sesión**: exigir cuenta para mirar sería pedir el DNI en la puerta del escaparate. Se generan **dos derivadas** por pieza, 1600 px para el visor y 600 px para la rejilla; sin la segunda, una cuadrícula de treinta piezas descarga las treinta a tamaño completo. Y aquí **re-encodificar sí es lo correcto**, al revés que en la entrega digital de D20: esto son imágenes para mirar en pantalla, no para imprimir. |
| **D34** | **La pantalla que vende va delante del registro; la foto va por estilo** | Dos cambios que salen de la misma auditoría de UX del 2026-08-20 y del mismo error: pedir una decisión antes de haber dado con qué tomarla. **`/live-art` era la única pantalla de venta detrás de `RutaProtegida`**, y es el servicio de más margen: quien llegaba desde Instagram —que es de donde llega— se encontraba un formulario de acceso en lugar de la propuesta. N18 no se relaja, cambia de sitio: la ruta es pública, el formulario se sustituye por la invitación a crear cuenta cuando no hay sesión, y `POST /api/events` la sigue exigiendo. La consecuencia técnica es que `useEventos` necesita un interruptor: la pantalla se monta ahora sin sesión y el endpoint responde 401. **Y `product_variants` gana su propia `image_media_id`**, porque la ficha pedía elegir entre «Diseño de moda», «Acuarela» y «Digital» —tres dibujos que no se parecen en nada— enseñando tres nombres y tres precios: la foto del producto era una sola y valía igual para los tres. Columna en la migración original y no apilada (D25), endpoint `multipart` aparte del JSON como el de D33 para el producto, y eager loading en las cuatro respuestas que pintan variantes. |
...
| # | Regla |
|---|---|
| **N18** | **El registro es obligatorio** para encargar. No hay compra como invitado. Lo que la regla exige es **la cuenta al enviar**, no el muro en la puerta: las pantallas que venden —el catálogo, la ficha, la galería y desde el 2026-08-20 también `/live-art`— se leen enteras sin sesión, y la cuenta se pide en el último paso, cuando ya se sabe para qué sirve. Lo comprueba el servidor (`auth:sanctum` y las Policies), nunca el router. |
...
### Galería y «Sobre mí» · ✅ completada (2026-08-19)

Las dos pantallas que el legacy tenía y la SPA nunca llegó a construir. La galería entra como **D33**: contenido que gestiona Felicitas, no catálogo.

| Entregado | Estado |
|---|---|
| `gallery_pieces` + `storeGalleryImage()` con sus dos derivadas | ✅ `9f44ec7` |
| `GET /api/gallery` público · CRUD y reorden en `/api/admin/gallery` | ✅ `9f44ec7` |
| Galería con filtros y visor, gestión en el backoffice, «Sobre mí» | ✅ `91222fa` |
| 13 tests de backend, 4 de SPA y 2 recorridos E2E | ✅ `7eb9deb` |

**Arranca vacía**, y es lo correcto: la alternativa era volcar las 135 fotos del archivo tal cual —repetidas, sin recortar y con nombres de WhatsApp— y dejar que cambiarlo exigiera un desarrollador, que es justo lo que D5 rechazó.
